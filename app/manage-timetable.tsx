...
import db, { getAllLecturers, getAllVenues } from '../database/db';
...
export default function ManageTimetable() {
  ...
  const loadAll = () => {
    const t = db.getAllSync(`
      SELECT timetable.*, lecturers.name as lec_name, lecturers.surname as lec_surname,
    ...
    const v = getAllVenues();
    if (v.success) setVenues(v.venues);
  };

  const checkClash = (lecturerId: string, venueId: string, day: string, start: string, end: string) => {
    const clashes = db.getAllSync(`
      SELECT * FROM timetable
      WHERE day = ? AND (
        (lecturer_id = ? OR venue_id = ?) AND
        NOT (end_time <= ? OR start_time >= ?)
      )
    `, [day, lecturerId, venueId, start, end]);
  ...
  const handleAdd = () => {
    ...
    const hasClash = checkClash(selectedLecturer, selectedVenue, day, startTime, endTime);

    if (hasClash) {
      setClashWarning('Warning: This time slot clashes with an existing class!');
      return;
    }

    try {
      db.runSync(
        `INSERT INTO timetable (module, lecturer_id, venue_id, day, start_time, end_time, program)
         VALUES (?, ?, ?, ?, ?, ?, ?)`,
        [module, parseInt(selectedLecturer), parseInt(selectedVenue), day, startTime, endTime, program]
      );
      ...
      Alert.alert('Error', 'Could not add to timetable');
    }
  };
    ...
    <ScrollView style={styles.container}>
      ...
      {showForm && (
        <View style={styles.form}>
          ...
          {lecturers.length === 0 ? (
            <Text style={styles.noDataText}>No lecturers added yet</Text>
          ) : (
            lecturers.map((l: any) => (
              <TouchableOpacity
                key={l.id}
                style={[styles.selectItem, selectedLecturer === String(l.id) && styles.selectItemActive]}
                onPress={() => setSelectedLecturer(String(l.id))}
              >
          ...
          {venues.length === 0 ? (
            <Text style={styles.noDataText}>No venues added yet</Text>
          ) : (
            venues.map((v: any) => (
              <TouchableOpacity
                key={v.id}
                style={[styles.selectItem, selectedVenue === String(v.id) && styles.selectItemActive]}
                onPress={() => setSelectedVenue(String(v.id))}
              >
      ...
      {timetable.length === 0 ? (
        <View style={styles.emptyBox}>
          ...
          <Text style={styles.emptyText}>
            Tap the + button above to add your first class
          </Text>
        </View>
      ) : (
        days.map((d) => {
                ...
                <View key={t.id} style={styles.classCard}>
                  ...
                  <View style={styles.classInfo}>
                    ...
                    <Text style={styles.classDetail}>
                      <Ionicons name="person-outline" size={12} color="#a0c4ff" /> {t.lec_name} {t.lec_surname}
                    </Text>
                    <Text style={styles.classDetail}>
                      <Ionicons name="location-outline" size={12} color="#a0c4ff" /> {t.venue_name}
                    </Text>
                  </View>
                </View>
              ))}
