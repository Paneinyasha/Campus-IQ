...
export default function StudentLogin() {
  ...
  const validateEmail = (e: string) => /^R\d{7}R@students\.msu\.ac\.zw$/i.test(e.trim());
  ...
  const strengthColor = () => {
    if (strength === 'Weak') return '#D85A30';
  ...
  const handleLogin = async () => {
    if (loading) return;
    if (!loginReg || !loginPassword) { Alert.alert('Missing', 'Please enter your reg number and password'); return; }
    ...
    try {
      ...
      if (result.success) {
        if (rememberMe) {
          await AsyncStorage.setItem('student_remember', JSON.stringify({ reg: loginReg, pwd: loginPassword }));
        } else {
  ...
  const handleSignup = async () => {
    ...
    if (!validateEmail(email)) { Alert.alert('Invalid Email', 'Email must be R2211952R@students.msu.ac.zw'); return; }
    ...
    setLoading(true);
    try {
      const result = await registerStudent(name.trim(), surname.trim(), program.trim(), regNumber.trim().toUpperCase(), email.trim().toLowerCase(), password, phone.trim());
      if (result.success) {
        ...
        if (result.otp) {
          ...
          Alert.alert(
            'Account Created!',
            `Your verification code is:\n\n${result.otp}\n\nEnter this code on the next screen.\n\n(In production, this will be sent via email. Save this code now.)`,
            [{ text: 'Got it', onPress: () => setScreen('verify') }]
          );
        } else if (result.resent) {
          Alert.alert('Code Resent', 'A new verification code has been generated. Check below for your code.', [{ text: 'OK', onPress: () => setScreen('verify') }]);
        } else {
          setScreen('verify');
        }
      } else {
  ...
  const handleVerify = async () => {
    if (loading) return;
    if (!otpCode || otpCode.length !== 6) { Alert.alert('Invalid', 'Please enter the 6-digit code'); return; }
    setLoading(true);
    try {
  ...
  const handleResend = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.otp) {
          setDevOtp(result.otp);
          Alert.alert('New Code Generated', `Your new verification code is:\n\n${result.otp}\n\nSave this code now.`);
        } else {
          Alert.alert('Code Resent', 'A new code has been generated.');
        }
      } else {
    ...
    return (
      <ScrollView contentContainerStyle={styles.container}>
        ...
        <View style={styles.infoBox}>
          ...
          <Text style={styles.infoText}>Save your code as soon as it appears. Codes expire in 10 minutes.</Text>
        </View>
  ...
  if (screen === 'signup') {
    return (
      <ScrollView contentContainerStyle={styles.container}>
        ...
        {[
          { placeholder: 'First Name', value: name, setter: setName, cap: 'words' as any },
          { placeholder: 'Surname', value: surname, setter: setSurname, cap: 'words' as any },
          { placeholder: 'Program e.g. BSc Computer Science', value: program, setter: setProgram, cap: 'words' as any },
        ].map((f) => (
          <View key={f.placeholder} style={styles.inputBox}>
            <Ionicons name="person-outline" size={20} color="#1D9E75" style={styles.inputIcon} />
            <TextInput style={styles.input} placeholder={f.placeholder} placeholderTextColor="#aaa" value={f.value} onChangeText={f.setter} autoCapitalize={f.cap} />
          </View>
        ))}
        ...
        <View style={styles.infoBox}>
          ...
          <Text style={styles.infoText}>After signup you will receive a verification code. Save it immediately.</Text>
        </View>
...
const styles = StyleSheet.create({
  ...
  verifyIconBox: { width: 100, height: 100, borderRadius: 50, backgroundColor: '#0a3d2e', alignItems: 'center', justifyContent: 'center', borderWidth: 3, borderColor: '#1D9E75', marginBottom: 16 },
