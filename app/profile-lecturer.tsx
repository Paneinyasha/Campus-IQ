...
export default function ProfileLecturer() {
  ...
  const saveAvatar = async () => {
    try {
      await supabase.from('lecturers').update({ avatar_id: selectedAvatar }).eq('id', lecturer.id);
  ...
  const changePassword = async () => {
    ...
    if (!validatePassword(newPassword)) {
      Alert.alert('Weak Password', 'New password must be 8+ characters with uppercase, lowercase, number and special character'); return;
    }
    if (newPassword !== confirmPassword) {
      Alert.alert('Mismatch', 'New passwords do not match'); return;
    }
    try {
      const { data } = await supabase.from('lecturers').select('*').eq('id', lecturer.id).eq('password', currentPassword).single();
      if (!data) { Alert.alert('Error', 'Current password is incorrect'); return; }
      await supabase.from('lecturers').update({ password: newPassword, must_change_password: 0 }).eq('id', lecturer.id);
      const updated = { ...lecturer, must_change_password: 0 };
      await AsyncStorage.setItem('current_lecturer', JSON.stringify(updated));
      setCurrentPassword(''); setNewPassword(''); setConfirmPassword(''); setPasswordStrength('');
  ...
  return (
    <ScrollView style={styles.container}>
      ...
      <View style={styles.profileBox}>
        ...
        {lecturer?.phone && <Text style={styles.profilePhone}>{lecturer?.phone}</Text>}
      </View>
      ...
      <View style={styles.inputBox}>
        ...
        <TextInput style={styles.input} placeholder="Current Password" placeholderTextColor="#aaa" value={currentPassword} onChangeText={setCurrentPassword} secureTextEntry={!showCurrent} />
      ...
      <View style={styles.inputBox}>
        <Ionicons name="lock-closed-outline" size={20} color="#534AB7" style={styles.inputIcon} />
        <TextInput style={styles.input} placeholder="New Password" placeholderTextColor="#aaa" value={newPassword}
          onChangeText={(text) => { setNewPassword(text); setPasswordStrength(getPasswordStrength(text)); }}
...
const styles = StyleSheet.create({
  ...
  profileBox: { alignItems: 'center', backgroundColor: '#0a2a4a', borderWidth: 1, borderColor: '#534AB7', borderRadius: 16, padding: 24, marginBottom: 24 },
  ...
  profileDept: { fontSize: 13, color: '#a0c4ff', marginBottom: 4 },
  ...
  saveAvatarBtn: { backgroundColor: '#534AB7', padding: 14, borderRadius: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 24 },
  ...
  changePasswordBtn: { backgroundColor: '#1D9E75', padding: 14, borderRadius: 12, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, marginBottom: 40 },
