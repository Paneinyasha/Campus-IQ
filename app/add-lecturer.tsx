...
export default function AddLecturer() {
  ...
  const handleAdd = async () => {
    ...
    setLoading(true);
    try {
      const result = await addLecturer(name, surname, department, email, password, phone);
      if (result.success) {
        Alert.alert('Lecturer Added!', `Account created for ${name} ${surname}.\n\nLogin details:\nEmail: ${email}\nTemp Password: ${password}\n\nThey will be asked to change password on first login.`);
        setName(''); setSurname(''); setDepartment(''); setEmail(''); setPhone(''); setPassword('');
      } else {
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <View style={styles.noticebox}>
        ...
        <Text style={styles.noticeText}>The lecturer will be required to change their password on first login</Text>
      </View>
      ...
      <View style={styles.inputBox}>
        ...
        <TextInput style={styles.input} placeholder="Department e.g. Computer Science" placeholderTextColor="#aaa" value={department} onChangeText={setDepartment} />
      </View>
      <View style={styles.inputBox}>
      ...
      <View style={styles.inputBox}>
        ...
        <TextInput style={styles.input} placeholder="Temporary Password e.g. MSU1234" placeholderTextColor="#aaa" value={password} onChangeText={setPassword} secureTextEntry={!showPassword} />
      ...
      <View style={styles.hintBox}>
        ...
        <Text style={styles.hintText}>Share the email and temporary password with the lecturer. They will set a new password on first login.</Text>
      </View>
      <TouchableOpacity style={[styles.btn, loading && { opacity: 0.6 }]} onPress={handleAdd} disabled={loading}>
        <Ionicons name="checkmark-circle-outline" size={22} color="#ffffff" />
        <Text style={styles.btnText}>{loading ? 'Creating...' : 'Create Lecturer Account'}</Text>
      </TouchableOpacity>
...
const styles = StyleSheet.create({
  container: { flexGrow: 1, backgroundColor: '#1a1a2e', alignItems: 'center', justifyContent: 'center', padding: 24, paddingTop: 60, paddingBottom: 40 },
  ...
  noticebox: { flexDirection: 'row', alignItems: 'flex-start', backgroundColor: '#2a1500', borderWidth: 1, borderColor: '#FFD700', padding: 14, borderRadius: 12, marginBottom: 20, gap: 10 },
